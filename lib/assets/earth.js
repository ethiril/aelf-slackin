$( document ).ready(function earth() {
  ...
  //if(!device.android()){
    ...
    var mapDiv = document.getElementById("particles2");
		console.log("Attached to #particles2"); // Testing if this code gets triggered
    ...
    $(document).ready(function() {
      jQuery.get('./assets/data/density.csv', function(data) {
				console.log("converted CSV"); // Add log for testing
        addDensity(CSVToArray(data));
        addLights();
        addEarth();
				console.log("added earth");
        render();
    ...
    function addDensity(data) {
      ...
      var cubeMat3 = new THREE.MeshBasicMaterial({
        color: 0xffffff,
        opacity: 0.6,
        emissive: 0xffffff
      });
      var cubeMat2 = new THREE.MeshBasicMaterial({
        color: 0x0F9CFF
      });
    ...
    function addClouds() {
      var spGeo = new THREE.SphereGeometry(400, 30, 30);
      var cloudsTexture = THREE.ImageUtils.loadTexture("./earth_clouds_1024.png");
